...
exports.getThrottledBatches = getThrottledBatches;
exports.getDailyLimit = getDailyLimit;
exports.getSuppressedSubscribers = getSuppressedSubscribers;
exports.orderByEngagement = orderByEngagement;
exports.generateUnsubToken = generateUnsubToken;
exports.verifyUnsubToken = verifyUnsubToken;
...
function scoreContent(html, subject) {
    const issues = [];
    let score = 0;
...
async function getThrottledBatches(subscriberIds, businessId) {
...
async function getDailyLimit(businessId) {
    const prisma = (0, prisma_1.getPrisma)();
    const warmup = await prisma.domainWarmup.findUnique({ where: { businessId } });
    if (!warmup || warmup.complete)
        return Infinity;
    // Auto-increment warmup day
    const daysSinceStart = Math.floor((Date.now() - warmup.startedAt.getTime()) / (86400 * 1000));
    if (daysSinceStart > warmup.currentDay) {
        const schedule = warmup.warmupSchedule;
...
async function getSuppressedSubscribers(listId) {
    const prisma = (0, prisma_1.getPrisma)();
...
async function orderByEngagement(subscriberIds) {
    const prisma = (0, prisma_1.getPrisma)();
...
function generateUnsubToken(subscriberId, secret) {
    return crypto.createHmac('sha256', secret).update(subscriberId).digest('hex').slice(0, 32);
}
function verifyUnsubToken(subscriberId, token, secret) {
...
//# sourceMappingURL=spamEngine.js.map