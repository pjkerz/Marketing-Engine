...
async function upsertSession(prisma, { businessId, sessionId, visitorId, affiliateCode, entryUrl, entryChannel, entryContentId, funnelStage, }) {
...
router.get('/ref/:code', async (req, res) => {
    ...
    const channel = req.query['ch'] || 'unknown';
    const contentRunId = req.query['cid'] || undefined;
    const ua = req.headers['user-agent'] || '';
    // Read or set visitorId cookie (1-year)
    let visitorId = req.cookies?.['_an_vid'];
    if (!visitorId) {
    ...
    setImmediate(async () => {
        try {
            ...
            logger_1.logger.warn({ err }, 'Funnel tracking error on ref click');
        }
    });
    // Immediate redirect
    const prisma = (0, prisma_1.getPrisma)();
    ...
    try {
        ...
        if (affiliate) {
            ...
            if (config?.landingPageUrl)
                landingUrl = config.landingPageUrl;
        }
    }
    catch { }
...
router.post('/event', async (req, res) => {
    ...
    setImmediate(async () => {
        try {
            ...
            logger_1.logger.warn({ err }, 'Funnel event tracking error');
        }
    });
});
...
router.get('/pixel.gif', async (req, res) => {
    ...
    setImmediate(async () => {
        try {
            ...
            logger_1.logger.warn({ err }, 'Email pixel tracking error');
        }
    });
});
...
router.get('/click/:trackingId', async (req, res) => {
    ...
    try {
        if (rawUrl)
            dest = decodeURIComponent(rawUrl);
    }
    catch { }
    res.redirect(302, dest);
    setImmediate(async () => {
        try {
            ...
            logger_1.logger.warn({ err }, 'Email click tracking error');
        }
    });
});
...
router.post('/conversion', async (req, res, next) => {
    ...
    setImmediate(async () => {
        try {
            ...
            logger_1.logger.warn({ err }, 'Conversion webhook tracking error');
        }
    });
});
