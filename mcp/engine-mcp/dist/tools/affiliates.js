...
async function getAffiliates(input) {
    const businessId = await (0, db_1.resolveBusinessId)(input.businessSlug);
    const where = input.activeOnly !== false ? `AND a.active = true` : '';
    const rows = await (0, db_1.getPool)().query(`SELECT a.id, a.code, a.name, a.email, a.active, a.created_at,
     ...
     GROUP BY a.id ORDER BY conversions DESC, clicks DESC`, [businessId]);
    return {
...
async function getAffiliateLeaderboard(input) {
    const businessId = await (0, db_1.resolveBusinessId)(input.businessSlug);
    const since = new Date(Date.now() - (input.days ?? 30) * 86400000);
    const rows = await (0, db_1.getPool)().query(`SELECT a.name, a.code, a.email,
            COUNT(DISTINCT fe.id) FILTER (WHERE fe.event_type='click') AS clicks,
            COUNT(DISTINCT ce.id) AS conversions,
     ...
     GROUP BY a.id ORDER BY conversions DESC, clicks DESC LIMIT 20`, [businessId, since]);
    return {
...
async function getCommissions(input) {
    const businessId = await (0, db_1.resolveBusinessId)(input.businessSlug);
    const statusFilter = input.status ? `AND c.status = $2` : '';
    const params = input.status ? [businessId, input.status] : [businessId];
    const rows = await (0, db_1.getPool)().query(`SELECT c.id, c.affiliate_code, c.amount, c.status, c.created_at,
