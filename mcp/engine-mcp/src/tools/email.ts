...
export async function getEmailLists(input: { businessSlug: string }) {
  const businessId = await resolveBusinessId(input.businessSlug);
  const rows = await getPool().query(
    `SELECT el.id, el.name, el.description, el.active,
     ...
     GROUP BY el.id ORDER BY el."createdAt" DESC`,
    [businessId],
  );
  return { lists: rows.rows, total: rows.rowCount };
}

export async function getCampaigns(input: { businessSlug: string; status?: string }) {
  const businessId = await resolveBusinessId(input.businessSlug);
  const statusFilter = input.status ? `AND c.status = $2` : '';
  const params = input.status ? [businessId, input.status] : [businessId];
  const rows = await getPool().query(
    `SELECT c.id, c.name, c.subject, c.status, c.sent_count, c.open_count,
...
export async function getEmailHealth(input: { businessSlug: string }) {
  const businessId = await resolveBusinessId(input.businessSlug);
  const configRow = await getPool().query(
    `SELECT "dailySendCap", "warmupComplete", "sendingDomain", "fromEmail"
     FROM business_configs WHERE "businessId" = $1`,
    [businessId],
  );
  const config = configRow.rows[0];

  const todaySent = await getPool().query<{ count: string }>(
    `SELECT COUNT(*) as count FROM email_campaigns
     WHERE "businessId" = $1 AND DATE("sentAt") = CURRENT_DATE AND status = 'sent'`,
    [businessId],
  );

  const bounceRate = await getPool().query<{ rate: string }>(
    `SELECT ROUND(AVG(bounce_count::numeric / NULLIF(sent_count,0)) * 100, 2) AS rate
     FROM email_campaigns WHERE "businessId" = $1 AND sent_count > 0`,
    [businessId],
  );
...
export async function getDripSequences(input: { businessSlug: string }) {
  const businessId = await resolveBusinessId(input.businessSlug);
  const rows = await getPool().query(
    `SELECT s.id, s.name, s.status, s.trigger_event,
