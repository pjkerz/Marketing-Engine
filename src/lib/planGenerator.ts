...
export async function generateWeekPlan(
  ...
  businessId: string,
  weekStartDate: Date
): Promise<{ planId: string; slotsCreated: number }> {
  ...
  if (existing) return { planId: existing.id, slotsCreated: existing.slots.length };
  ...
  const platforms: string[] = instance?.preferredPlatforms?.length
    ? instance.preferredPlatforms
    : ['linkedin'];
  const preferredTimes = (instance?.preferredPostTimes ?? {}) as Record<string, string[]>;
  ...
  const optTimes = optRule?.active ? (optRule.config as Record<string, string[]>) : {};

  // Distribute weeklyTarget slots across platforms and 7 days
  const slots: Array<{ platform: string; scheduledDate: Date }> = [];
  let dayIndex = 0;
