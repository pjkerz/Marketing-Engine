...
const COMPETITION_SCORE: Record<string, number> = { low: 1, medium: 2, high: 3 };

export function scoreKeyword(kw: KeywordIdea): number {
  if (!kw.monthlyVolume || !kw.cpcEstimate || kw.cpcEstimate === 0) return 0;
...
export function mergeKeywordIdeas(lists: KeywordIdea[][]): KeywordIdea[] {
  const merged: Map<string, KeywordIdea> = new Map();
