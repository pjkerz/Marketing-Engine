...
const pendingOAuth = new Map<string, { affiliateCode: string; codeVerifier?: string }>();

function genState(affiliateCode: string, extra: { codeVerifier?: string } = {}): string {
...
async function saveConnection(affiliateCode: string, platform: string, tokens: Record<string, any>): Promise<void> {
  const prisma = getPrisma();
  const affiliate = await prisma.affiliate.findUnique({ where: { code: affiliateCode } });
  if (!affiliate) throw new Error(`Affiliate not found: ${affiliateCode}`);
...
function successPage(affiliateCode: string, platform: string): string {
  return `<!DOCTYPE html>
<html>
...
function errorPage(msg: string, affiliateCode: string): string {
  return `<!DOCTYPE html>
<html>
...
router.get('/auth/linkedin/start', (req: Request, res: Response) => {
  const affiliateCode = req.query['affiliateCode'] as string;
  if (!affiliateCode) { res.status(400).send('Missing affiliateCode'); return; }
  if (!env.LI_CLIENT_ID) { res.status(503).send('LinkedIn OAuth not configured'); return; }
...
router.get('/auth/linkedin/callback', async (req: Request, res: Response) => {
  ...
  if (!stored) { res.send(errorPage('Session expired — please try again.', '')); return; }
  pendingOAuth.delete(state);
  if (error || !code) { res.send(errorPage(error || 'No code received', stored.affiliateCode)); return; }
...
router.get('/auth/facebook/start', (req: Request, res: Response) => {
  const affiliateCode = req.query['affiliateCode'] as string;
  if (!affiliateCode) { res.status(400).send('Missing affiliateCode'); return; }
  if (!env.FB_APP_ID) { res.status(503).send('Facebook OAuth not configured'); return; }
...
router.get('/auth/twitter/start', (req: Request, res: Response) => {
  const affiliateCode = req.query['affiliateCode'] as string;
  if (!affiliateCode) { res.status(400).send('Missing affiliateCode'); return; }
  if (!env.X_CLIENT_ID) { res.status(503).send('X/Twitter OAuth not configured'); return; }
...
router.get('/auth/twitter/callback', async (req: Request, res: Response) => {
  ...
  const basicAuth = Buffer
    .from(`${env.X_CLIENT_ID}:${env.X_CLIENT_SECRET}`)
    .toString('base64');
...
router.get('/auth/youtube/start', (req: Request, res: Response) => {
  const affiliateCode = req.query['affiliateCode'] as string;
  if (!affiliateCode) { res.status(400).send('Missing affiliateCode'); return; }
  if (!env.GOOGLE_CLIENT_ID) { res.status(503).send('Google/YouTube OAuth not configured'); return; }
...
router.get('/auth/reddit/start', (req: Request, res: Response) => {
  const affiliateCode = req.query['affiliateCode'] as string;
  if (!affiliateCode) { res.status(400).send('Missing affiliateCode'); return; }
  if (!env.REDDIT_CLIENT_ID) { res.status(503).send('Reddit OAuth not configured'); return; }
...
router.get('/auth/reddit/callback', async (req: Request, res: Response) => {
  const { code, state, error } = req.query as Record<string, string>;
  const stored = pendingOAuth.get(state);
...
router.delete('/auth/:platform/disconnect', async (req: Request, res: Response) => {
  const { platform } = req.params as { platform: string };
  const affiliateCode = req.query['affiliateCode'] as string;
  if (!affiliateCode) { res.status(400).json({ error: 'Missing affiliateCode' }); return; }
