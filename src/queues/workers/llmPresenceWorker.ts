...
import { queryAllLlms, LlmName } from '../../modules/llmPresence/llmQueryClients';
import { analyseResponse } from '../../modules/llmPresence/responseAnalyser';

const GROQ_API_KEY = process.env['GROQ_API_KEY'] ?? '';

interface LlmPresenceJob {
  auditId: string;
  businessId: string;
}
...
async function runPresenceAudit(job: { data: LlmPresenceJob }): Promise<void> {
  ...
  try {
    ...
    const competitors: string[] = (audit.competitors as string[]) ?? [];
    const customQueries = (audit.queries as unknown as QueryItem[]) ?? [];
    const queries: QueryItem[] = customQueries.length > 0 ? customQueries : DEFAULT_QUERIES;

    const llmNames: LlmName[] = ['chatgpt', 'claude', 'gemini', 'perplexity'];
    const BATCH_SIZE = 5;
    let totalResults = 0;
    let brandMentions = 0;
    ...
    for (let batchStart = 0; batchStart < queries.length; batchStart += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(async ({ query, category }) => {
        ...
        const llmResponses = await queryAllLlms(query);
        ...
        for (const llmName of llmNames) {
          ...
          const analysis = await analyseResponse(response, brandName, competitors);
          if (analysis.mentionsBrand) brandMentions++;
          ...
          await prisma.llmPresenceResult.create({
            data: {
              ...
              mentionsCompetitors: analysis.mentionsCompetitors as unknown as Parameters<typeof prisma.llmPresenceResult.create>[0]['data']['mentionsCompetitors'],
    ...
    const llmRates: Record<string, number> = {};
    ...
    const gapQueries = allResults
      .filter(r => !r.mentionsBrand && (r.mentionsCompetitors as Array<{ name: string }>).length > 0)
      .map(r => r.query)
      .slice(0, 10);
    ...
    logger.info({ module: 'llmPresenceWorker', auditId, brandMentions, totalResults }, 'LLM presence audit complete');
  } catch (err) {
    await prisma.llmPresenceAudit.update({ where: { id: auditId }, data: { status: 'failed' } }).catch(() => {});
    logger.error({ module: 'llmPresenceWorker', auditId, err }, 'LLM presence audit failed');
    throw err;
  }
}

let worker: Worker | null = null;
export let llmPresenceQueue: Queue | null = null;

export function startLlmPresenceWorker(): void {
  if (worker) return;
  const connection = getBullRedis();
  llmPresenceQueue = new Queue('v2-llm-presence', {
    connection,
    defaultJobOptions: { removeOnComplete: 10, removeOnFail: 20 },
  });
  worker = new Worker('v2-llm-presence', runPresenceAudit, { connection, concurrency: 1 });
  worker.on('failed', (job, err) => {
    logger.error({ module: 'llmPresenceWorker', auditId: job?.data?.auditId, err }, 'Job failed');
  });
  logger.info({ module: 'llmPresenceWorker' }, 'LLM presence worker started');
}

export async function stopLlmPresenceWorker(): Promise<void> {
  if (worker) { await worker.close(); worker = null; }
  if (llmPresenceQueue) { await llmPresenceQueue.close(); llmPresenceQueue = null; }
}
